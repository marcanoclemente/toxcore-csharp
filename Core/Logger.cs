...
namespace Toxcore.Core
{
    ...
    public static class Logger
    {
        ...
        public static string SafeKeyThumb(byte[] key, int visibleBytes = 8)
        {
            try
            {
                if (key == null) return "<null>";
                int n = Math.Min(Math.Max(visibleBytes, 1), 16);
                if (key.Length <= n)
                    return BitConverter.ToString(key).Replace("-", "");
                // show first n bytes only
                var slice = new byte[n];
                Buffer.BlockCopy(key, 0, slice, 0, n);
                return BitConverter.ToString(slice).Replace("-", "") + "..";
            }
            catch
            {
                return "<safe-thumb-failed>";
            }
        }

        // Returns a redacted representation of any byte[] that might contain secrets
        public static string RedactIfSecret(byte[] maybeSecret, bool isSecret = true)
        {
            if (!isSecret) return maybeSecret == null ? "<null>" : BitConverter.ToString(maybeSecret).Replace("-", "");
            return maybeSecret == null ? "<null>" : ("[REDACTED:" + SafeKeyThumb(maybeSecret) + "]");
        }

        // Convenience logging overloads to make it harder to accidentally log secrets
        public static void InfoKey(string message, byte[] key,
            [CallerFilePath] string file = "",
            [CallerMemberName] string member = "",
            [CallerLineNumber] int line = 0)
        {
            Logger.Log.InfoF($"{message} {RedactIfSecret(key)}", file, member, line);
        }
